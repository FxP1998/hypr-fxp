...
if ! command -v auto-cpufreq &> /dev/null; then
    ...
    run_as "$TARGET_USER" "cd $WORKDIR && git clone https://aur.archlinux.org/auto-cpufreq.git && cd auto-cpufreq && makepkg -s --noconfirm"
    
    echo -e "   - Installing as Root..."
    cd "$WORKDIR/auto-cpufreq" && pacman -U --noconfirm *.pkg.tar.zst
fi
...
if pacman -Qs jack2 > /dev/null; then
    pacman -Rdd --noconfirm jack2 &> /dev/null
fi
